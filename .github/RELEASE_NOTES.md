...
- Editor and terminal areas now support right-click context menus backed by the same real `CommandAction` path as shortcuts and the command palette. Editor menus cover save, clipboard, search, navigation, rename, suggestions, formatting, comments, run-in-terminal, path-copy, revert, and close-tab actions; terminal menus cover copy/paste/search, clear/restart, session management, maximize/resize, and focus actions when the child app is not using terminal mouse reporting.
