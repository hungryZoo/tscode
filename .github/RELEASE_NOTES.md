...
- Optional installed-language-server integration now speaks stdio JSON-RPC/LSP for `textDocument/hover`, `textDocument/definition`, `textDocument/references`, `textDocument/rename`, and `textDocument/completion`. Rust, Python, TypeScript/JavaScript, Go, and C-family files try common servers such as `rust-analyzer`, `pyright-langserver`, `typescript-language-server`, `gopls`, and `clangd`, while `TSCODE_LSP_COMMAND` can override the server for custom setups.
