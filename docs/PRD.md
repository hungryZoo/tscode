...
- As a developer, I can undo and redo edits, paste text, repeat search matches, list open editor tabs including dirty and Untitled buffers, close clean tabs immediately, close all/other/right-side clean editor tabs in one action while preserving dirty tabs, choose whether to save, discard, or cancel when closing a dirty tab, and reopen the most recently closed editor when I closed the wrong tab.
...
- `Ctrl-PageDown` and `Ctrl-PageUp` switch to next/previous editor tabs outside terminal focus and next/previous terminal sessions in terminal focus
- ``Ctrl-Shift-` ``/`F7`, `Ctrl-PageDown`/`F8`, `Ctrl-PageUp`, and `F9` create a new terminal, switch to the next terminal, switch to the previous terminal, and close the active terminal
