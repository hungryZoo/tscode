...
The command palette shall include Source Control, Stage All Changes, and Unstage All Changes actions. When the workspace is inside a Git repository, Source Control shall refresh Git status, open a quick panel listing changed files, include diff hunk entries parsed from `git diff --unified=0`, include stage/unstage command rows for paths whose porcelain index/worktree state supports that operation, support filtering those entries with the quick-panel query, open selected changed-file rows as read-only Git diff tabs, synthesize a readable new-file diff for untracked text files, prevent editing/saving those diff tabs, stage selected paths through `git add`, unstage selected paths through `git restore --staged` with reset/rm-cached fallbacks, stage all changes through `git add -A`, unstage all changes, refresh the panel after each operation, and open an existing changed file at the selected hunk line. Deleted or otherwise missing files shall remain visible as changed-file entries without crashing when selected.
...
- Modify a tracked Git file, open Source Control, confirm changed-file and hunk rows appear, confirm selecting a changed-file row opens a read-only diff tab, confirm stage/unstage rows update `git diff --cached`, and confirm the hunk row opens the file at the changed line.
