...
The command palette shall include Source Control, Stage All Changes, Unstage All Changes, and Discard All Changes actions. When the workspace is inside a Git repository, Source Control shall refresh Git status, open a quick panel listing changed files, include diff hunk entries parsed from `git diff --unified=0`, include stage/unstage/discard command rows for paths whose porcelain index/worktree state supports that operation, support filtering those entries with the quick-panel query, open selected changed-file rows as read-only Git diff tabs, synthesize a readable new-file diff for untracked text files, prevent editing/saving those diff tabs, stage selected paths through `git add`, unstage selected paths through `git restore --staged` with reset/rm-cached fallbacks, stage all changes through `git add -A`, unstage all changes, discard selected tracked changes through `git restore` or compatible fallbacks, remove selected untracked files through `git clean`, discard all changed paths after typed confirmation, block discard operations when an affected file-backed editor tab has unsaved changes, close clean editor tabs for files removed by discard, refresh the explorer and Source Control panel after each operation, and open an existing changed file at the selected hunk line. Deleted or otherwise missing files shall remain visible as changed-file entries without crashing when selected.
